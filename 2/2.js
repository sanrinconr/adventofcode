...
function Clave(letra, minCant, maxCant, clave){
    /*9-13 p: bppxpjpmpwcpppdprpp*/
...
Clave.prototype.esValida= function(){
    ...
    for (let i = 0; i < this.clave.length; i++) {
        if(this.clave[i] === this.letra){
            cantElementos++;
            
        }
    }
    if(cantElementos >= this.min && cantElementos <= this.max){
        return true
    }
    return false;
}
...
function __procesarPalabra(string){
    caracterActual = 0;
    var determinarMin= function(){
        minimo = ""
        while (string[caracterActual] !== "-") {
            minimo+=string[caracterActual]
            caracterActual++;
        }
    ...
    var determinarMax= function(){
        let max = ""
        while (string[caracterActual] !== " ") {
            max+=string[caracterActual]
            caracterActual++;
        }
        //Como se encuentra en " " se suma uno mas
        caracterActual++;
    ...
    let min = determinarMin()
    let max = determinarMax()
