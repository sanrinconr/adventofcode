...
function Clave(letra, pos1, pos2, clave){
    /*9-13 p: bppxpjpmpwcpppdprpp*/

    /*9*/
    this.pos1 = pos1-1; /*Se le resta ya que ellos no tienen en cuenta que el indice empieza en 0)*/
    /*13*/
    this.pos2 = pos2-1;
    /*p*/
    this.letra = letra;
...
Clave.prototype.esValida= function(){
    var pos1contieneLetra =  false;
    var pos2contieneLetra = false;
    if(this.clave[this.pos1] === this.letra){
        pos1contieneLetra = true;
    }
    if(this.clave[this.pos2] === this.letra ){
        pos2contieneLetra = true
    }
    var xor = function(a,b) {
        return ( a || b ) && !( a && b );
    }
    return xor(pos1contieneLetra,pos2contieneLetra)
...
function formatear(str){
    ...
    for (let i = 0; i < str.length; i++) {
       if (str[i] !== "\n") {
           ...
           arregloClaves.push(__procesarPalabra(actual))
           actual = ""
       }
    }
    return arregloClaves
}
function __procesarPalabra(string){
    caracterActual = 0;
    var determinarPos1= function(){
        pos1 = ""
        while (string[caracterActual] !== "-") {
            pos1+=string[caracterActual]
            caracterActual++;
        }
        //Como se encuentra en "-" se suma uno mas
        caracterActual++;
        return parseInt(pos1)
    }
    var determinarPos2= function(){
        let pos2 = ""
        while (string[caracterActual] !== " ") {
            pos2 +=string[caracterActual]
            caracterActual++;
        }
        //Como se encuentra en " " se suma uno mas
        caracterActual++;
    ...
    var determinarLetra= function(){
        let letra = ""
        while (string[caracterActual] !== ":") {
            letra+=string[caracterActual]
            caracterActual++;
        }
        //Como se encuentra en ":" se suman dos mas para pasar : y el espacio
        caracterActual+=2;
        return letra
    }
    var determinarPalabra = function(){
        let palabra = ""
        while (string[caracterActual] !== "\n" && caracterActual < string.length) {
            palabra+=string[caracterActual]
            caracterActual++;
        }
        return palabra
    }
    let min = determinarPos1()
    let max = determinarPos2()
    let letra = determinarLetra()
    let palabra = determinarPalabra()
    return new Clave(letra,min,max,palabra)

}


var claves = formatear (a)
var validas = 0;
for (let i = 0; i < claves.length; i++) {
    if(claves[i].esValida() === true){
        /*console.log("valida: ")
