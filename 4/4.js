var real = `eyr:2024 pid:662406624 hcl:#cfa07d byr:1947 iyr:2015 ecl:amb hgt:150cm
...
function individualizarPasaportes(entrada) {
    entrada += "\n"
    var salida = []
    var pasaporteActual = ""
    for (let i = 0; i < entrada.length; i++) {
        if (entrada[i] !== "\n") {
            ...
            if (entrada[i] === "\n") {
                //Si aun no se a llegado al final
                if (i < entrada.length - 1) {
                    //Si despues del salto de linea esta otro salto de linea entonces se termino el pasaporte
                    if (entrada[i + 1] === "\n") {
                        salida.push(pasaporteActual)
                        pasaporteActual = ""
                    } else {
                        //Para evitar que le agregue comas si el pasaporte esta recien creado
                        if (pasaporteActual !== "") {
                            pasaporteActual += ","
                        }
                    }
                    //Si se encontro un /n y se esta en el final obviamente sera el final del pasaporte
                } else {
                    salida.push(pasaporteActual)
                    pasaporteActual = ""
                }
...
/**
 * buscarClave - A partir de un texto dado en formato clave:valor
 * se retorna el valor
 *
 * @param  {string} clave clave a buscar
 * @param  {string} texto texto en el cual buscar
 * @return {string}       valor retornado
 */
function buscarClave(clave, texto) {
    var valor = "";
    var inicio = texto.search(clave)
    if (inicio === -1) {
        return null;
    }
    //Incluye la clave y el :
    inicio = inicio + clave.length + 1;
 ...
 *
 * @param  {type} byr description
 * @param  {type} iyr description
 * @param  {type} eyr description
 * @param  {type} hgt description
 * @param  {type} hcl description
 * @param  {type} ecl description
 * @param  {type} pid description
 * @param  {type} cid description
 * @return {type}     description
 */
function Pasaporte(byr, iyr, eyr, hgt, hcl, ecl, pid, cid) {
...
Pasaporte.prototype.valByr = function() {
    if (this.byr >= 1920 && this.byr <= 2002) {
        return true;
    }
    return false;
}
Pasaporte.prototype.valIyr = function() {
    if (this.iyr >= 2010 && this.iyr <= 2020) {
        return true;
    }
    return false;
}
Pasaporte.prototype.valEyr = function() {
    if (this.eyr >= 2020 && this.eyr <= 2030) {
        return true;
    }
    return false;
}
Pasaporte.prototype.valHgt = function() {
    if (this.hgt === null) {
        return false
    }
    var numero = parseInt(this.hgt)
    var letras = this.hgt.substring(((numero.toString().length)), (this.hgt.length));
    if (letras === "in") {
...
Pasaporte.prototype.valHcl = function() {
    if (this.hcl === null) {
        return false
    }
    var validar = function(str) {
        ...
        for (let i = 1; i < str.length; i++) {
            //Que sea un numero o una letra
            var numero = str.charCodeAt(i) >= 48 && str.charCodeAt(i) <= 57
            var mayus = str.charCodeAt(i) >= 65 && str.charCodeAt(i) <= 70
            var minus = str.charCodeAt(i) >= 97 && str.charCodeAt(i) <= 102

            if (!(numero === true || mayus === true || minus === true)) {
                return false
            }
    ...
    return validar(this.hcl)
}
Pasaporte.prototype.valEcl = function() {
    if (this.ecl === null) {
        return false
    }
    var validos = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
    var valido = false;
    for (let k = 0; k < validos.length; k++) {
        if (this.ecl === validos[k]) {
            valido = true;
        }
    }
    return valido
}
Pasaporte.prototype.valPid = function() {
    if (this.pid === null) {
        return false
    }
    var valido = true;
    ...
    //Que sea un numero
    for (let k = 0; k < this.pid.length; k++) {
        if (!(this.pid[k].charCodeAt(0) >= 48 && this.pid[k].charCodeAt(0) <= 57)) {
            valido = false;
            break;
        }
...
Pasaporte.prototype.valCid = function() {
    if (this.cid === null) {
        return true
    }
    return true;
...
Pasaporte.prototype.valido = function() {
    var validos = []
    validos.push(this.valByr())
    validos.push(this.valIyr())
    validos.push(this.valEyr())
    validos.push(this.valHgt())
    validos.push(this.valHcl())
    validos.push(this.valEcl())
    validos.push(this.valPid())
    validos.push(this.valCid())
...
function cantPaspValidos(pasaportes) {
    cont = 0;
    console.log("cant pasp: " + pasaportes.length)
    for (let i = 0; i < pasaportes.length; i++) {
        if (pasaportes[i].esValido() === true) {
            cont++;
        }
    }
    return cont;
...
var pasaportesTxt = individualizarPasaportes(real)
var pasaportes = []

pasaportesTxt.forEach(function(pasp) {
    pasaportes.push(new Pasaporte(
        buscarClave("byr", pasp),
        buscarClave("iyr", pasp),
        buscarClave("eyr", pasp),
        buscarClave("hgt", pasp),
        buscarClave("hcl", pasp),
        buscarClave("ecl", pasp),
        buscarClave("pid", pasp),
        buscarClave("cid", pasp)
    ))
});
var contador = 0;
for (var i = 0; i < pasaportes.length; i++) {
    if (pasaportes[i].valido()) {
        contador++;
    }
}
